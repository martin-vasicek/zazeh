...
// MSG COLORS
var MSG_COLOR_BLACK = "#000000";
var MSG_COLOR_GREEN = "#3e8515";
var MSG_COLOR_NOTIMPORTANT = "#8f8d88";
var MSG_COLOR_WARNINGINFO = "#852424";
var MSG_COLOR_WARNING = "#e32727";
...
var Room = {
	...
	init: function(options) {
		...
		Notifications.notify(Room, _("the room is {0}", Room.TempEnum.fromInt($SM.get('game.temperature.value')).text), MSG_COLOR_NOTIMPORTANT);
		Notifications.notify(Room, _("the fire is {0}", Room.FireEnum.fromInt($SM.get('game.fire.value')).text), MSG_COLOR_NOTIMPORTANT);
	},
	...
	onArrival: function(transition_diff) {
		...
		if($SM.get('game.builder.level') == 3) {
			...
			Room.updateIncomeView();
			Notifications.notify(Room, _("the stranger is standing by the fire. she says she can help. says she builds things."), MSG_COLOR_BLACK, true);
		}
	...
	lightFire: function() {
		...
		if(wood < 5) {
			Notifications.notify(Room, _("not enough wood to get the fire going"), MSG_COLOR_WARNINGINFO, true);
	...
	stokeFire: function() {
		var wood = $SM.get('stores.wood');
		if(wood === 0) {
			Notifications.notify(Room, _("the wood has run out"), MSG_COLOR_WARNINGINFO, true);
	...
	onFireChange: function() {
		...
		Notifications.notify(Room, _("the fire is {0}", Room.FireEnum.fromInt($SM.get('game.fire.value')).text), MSG_COLOR_NOTIMPORTANT, false, true);
		if($SM.get('game.fire.value') > 1 && $SM.get('game.builder.level') < 0) {
			$SM.set('game.builder.level', 0);
			Notifications.notify(Room, _("the light from the fire spills from the windows, out into the dark"), MSG_COLOR_NOTIMPORTANT);
	...
	coolFire: function() {
		...
		if($SM.get('game.fire.value') <= Room.FireEnum.Flickering.value &&
			...
			Notifications.notify(Room, _("builder stokes the fire"), MSG_COLOR_NOTIMPORTANT, false, true);
			$SM.set('stores.wood', wood - 1);
			$SM.set('game.fire',Room.FireEnum.fromInt($SM.get('game.fire.value') + 1));
		}
	...
	adjustTemp: function() {
		...
		if($SM.get('game.temperature.value') > 0 && $SM.get('game.temperature.value') > $SM.get('game.fire.value')) {
			...
			Notifications.notify(Room, _("the room is {0}" , Room.TempEnum.fromInt($SM.get('game.temperature.value')).text), MSG_COLOR_NOTIMPORTANT, false, true);
		}
	...
	unlockForest: function() {
		...
		Outside.init();
		Notifications.notify(Room, _("the wind howls outside"), MSG_COLOR_NOTIMPORTANT);
		Notifications.notify(Room, _("the wood is running out"), MSG_COLOR_WARNINGINFO, true);
	...
	updateBuilderState: function() {
		...
		if(lBuilder === 0) {
			Notifications.notify(Room, _("a ragged stranger stumbles through the door and collapses in the corner"), MSG_COLOR_BLACK, true);
			...
			Notifications.notify(Room, msg, MSG_COLOR_BLACK, true);
			if(lBuilder < 3) {
	...
	buy: function(buyBtn) {
		...
		for(var k in cost) {
			...
			if(have < cost[k]) {
				Notifications.notify(Room, _("not enough " + k), MSG_COLOR_WARNING); // ne-e
				return false;
			} else {
		...
		Notifications.notify(Room, good.buildMsg, MSG_COLOR_GREEN); // postaveno
	...
	build: function(buildBtn) {
		...
		if($SM.get('game.temperature.value') <= Room.TempEnum.Cold.value) {
			Notifications.notify(Room, _("builder just shivers"), MSG_COLOR_WARNINGINFO);
			return false;
		}
		...
		Notifications.notify(Room, craftable.buildMsg, MSG_COLOR_GREEN); // postaveno
	...
	craftUnlocked: function(thing) {
		...
		if(!$SM.get('game.buildings["'+thing+'"]')){
			Notifications.notify(Room, craftable.availableMsg, MSG_COLOR_BLACK, true);
		}
		return true;
	},
		...
		for(var k in Room.Craftables) {
			...
			if(craftable.button == null) {
				...
				if(max && !craftable.button.hasClass('disabled')) {
					Notifications.notify(Room, craftable.maxMsg, MSG_COLOR_BLACK);
				}
			}
			if(max) {
		...
		for(var g in Room.TradeGoods) {
			...
			if(good.button == null) {
				...
				if(goodsMax && !good.button.hasClass('disabled')) {
					Notifications.notify(Room, good.maxMsg, MSG_COLOR_BLACK);
				}
			}
			if(goodsMax) {
